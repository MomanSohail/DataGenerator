...
import os
import pickle
from Configration.functions import load_neural_config
from Planner.data_loader import load_test_dataset
...
if torch.cuda.is_available():
    mlp.cuda()
obc, obstacles, paths, path_lengths = 0, 0, 0, 0


def intialIze(path, dimension):
    global obc, obstacles, paths, path_lengths
    obc, obstacles, paths, path_lengths = load_test_dataset(path, dimension)
    global mlp
    mlp = MLP(34 if dimension == 3 else 32, dimension)
    mlp.load_state_dict(torch.load('Models/trainModels/mlp_100_4000_PReLU_ae_dd_final.pkl'))
...
def is_reaching_target(start1, start2, dimension):
    s1 = np.zeros(dimension, dtype=np.float32)
    s1[0] = start1[0]
    s1[1] = start1[1]
    if dimension == 3:
        s1[2] = start1[2]

    s2 = np.zeros(dimension, dtype=np.float32)
    s2[0] = start2[0]
    s2[1] = start2[1]
    if dimension == 3:
        s2[2] = start2[2]
...
def main(dimension, progress_bar, dataset_path):
    # Create model directory
    args = load_neural_config()
    intialIze(dataset_path, dimension)
    ...
    for i in range(0, 1):
        ...
        for j in range(0, dimension):
            ...
            if path_lengths[i][j] > 0:
                ...
                goal = np.zeros(dimension, dtype=np.float32)
            ...
            progress_bar.setProperty("value", j / 3 * 100)
        tot.append(et)
